...
import { EtlJobsService, SaudeFila, WorkerTravado } from '../../services/etl-jobs.service';
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="font-semibold text-red-800 dark:text-red-300">
                {{ workersTravados().length === 1 ? 'Worker travado' : 'Workers travados' }}
              </p>
              <p class="text-sm text-red-700 dark:text-red-400 mt-1">
                @for (w of workersTravados(); track w.job) {
                  <span class="block">
                    <strong>{{ w.job }}</strong>: preso há {{ formatarIdade(w.minutosPreso) }} sem liberar — não deveria acontecer, o guard interno aborta em 8min.
                  </span>
                }
              </p>
              <p class="text-xs text-red-600 dark:text-red-500 mt-2">Reiniciar o container Albatroz.ETL.API no servidor libera na hora, mas isso é sintoma de bug — não deveria estar acontecendo.</p>
            </div>
          </div>
        </div>
      }
...
export class EtlVisaoGeralPageComponent {
  ...
  workersTravados = signal<WorkerTravado[]>([]);
  ...
  private carregarSaudeFilas(): void {
    this.jobsService.saudeFilas().subscribe(resultado => {
      if (resultado.dados) {
        this.saudeFilas.set(resultado.dados.filas);
        this.workersTravados.set(resultado.dados.travados);
      }
    });
  }
