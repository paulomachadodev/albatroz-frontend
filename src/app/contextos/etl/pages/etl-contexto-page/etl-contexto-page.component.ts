    ...
    <div class="p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4">
          ...
          <div class="text-xs text-slate-500 mt-1">elegiveis para reprocessar</div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4">
          ...
          <div class="text-xs text-slate-500 mt-1">falhas permanentes</div>
        </div>
      </div>
  ...
  pipeline = toSignal(
    this.#route.paramMap.pipe(
      switchMap(params => {
        ...
        return this.#jobsService.pipelineEntidade(ent).pipe(
          switchMap(resultado => resultado.dados ? [resultado.dados] : [])
        );
      })
    ),
    { initialValue: { entidade: '', pendentes: 0, processados: 0, erros: 0, deadLetter: 0 } as EtlPipelineEntidadeResposta }
  );
